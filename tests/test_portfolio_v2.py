...
class TestPortfolioV2Enhancements(unittest.TestCase):
    def setUp(self):
        ...
        self.html_file = self.portfolio_dir / "index.html"
        self.css_file = self.portfolio_dir / "styles.css"
        self.js_file = self.portfolio_dir / "app.js"
        self.robots_file = self.portfolio_dir / "robots.txt"
        self.sitemap_file = self.portfolio_dir / "sitemap.xml"
        self.workflow_file = self.root / ".github" / "workflows" / "deploy-pages.yml"
        self.deploy_script = self.root / "bin" / "deploy"
    ...
    def test_deployment_workflow_and_runner(self):
        self.assertTrue(self.workflow_file.exists(), "deploy-pages.yml workflow file missing")
        ...
        self.assertTrue(self.deploy_script.exists(), "bin/deploy runner script missing")
        self.assertTrue(os.access(self.deploy_script, os.X_OK), "bin/deploy must be executable")
